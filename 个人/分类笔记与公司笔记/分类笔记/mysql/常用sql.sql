...
alter table lead  change column recall recall int null  comment '是否撤回 1撤回 0不撤回'
...
select type,sum(case vender when 'A' then pcs else 0 end),sum(case vender when 'C' then pcs else 0 end),sum(case venderwhen 'B' then pcs else 0 end) FROMtablename group by type
